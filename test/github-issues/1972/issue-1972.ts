...
describe("github issues > #1972 STI problem - empty columns", () => {
    ...
    it("should insert with userId", () => Promise.all(connections.map(async connection => {
        ...
        const result = await connection.manager.findOneBy(TournamentUserParticipant, { id: tournamentUserParticipant.id });
        if (result) {
    ...
    it("should insert with ownerId", () => Promise.all(connections.map(async connection => {
        ...
        const result = await connection.manager.findOneBy(TournamentSquadParticipant, { id: tournamentSquadParticipant.id });
