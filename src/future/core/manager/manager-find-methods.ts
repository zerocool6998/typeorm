...
export interface ManagerFindMethods<Driver extends AnyDriver> {
  ...
  findOptions<Entity extends AnyEntity>(
    entityRef: () => Entity,
  ): FindOptionsBuilder<Entity>

  /**
   * Finds entities by a given criteria.
   */
  find<Entity extends AnyEntity, Where extends WhereOptions<Entity>>(
    entityRef: () => Entity,
    where: Where,
   ...
   */
  findBy<Entity extends AnyEntity, Options extends FindOptionsMany<Entity>>(
    entityRef: () => Entity,
    options: Options,
  ): Promise<
    FindReturnType<
   ...
   */
  findOne<Entity extends AnyEntity, Where extends WhereOptions<Entity>>(
    entityRef: () => Entity,
    where: Where,
   ...
   * If entity was not found in the database - it returns null.
   */
  findOneBy<Entity extends AnyEntity, Options extends FindOptions<Entity>>(
    entityRef: () => Entity,
    options: Options,
   ...
   */
  findOneOrFail<Entity extends AnyEntity, Where extends WhereOptions<Entity>>(
    entityRef: () => Entity,
    where: Where,
  ): Promise<FindReturnType<Entity, {}, false, "all">>
   ...
   */
  findAndCount<Entity extends AnyEntity, Where extends WhereOptions<Entity>>(
    entityRef: () => Entity,
    where: Where,
   ...
   * but ignores pagination settings ("skip" and "take" options).
   */
  findAndCountBy<Entity extends AnyEntity, Options extends FindOptions<Entity>>(
    entityRef: () => Entity,
    options: Options,
  ): Promise<
    [
   ...
   * Counts entities matching given where criteria.
   */
  count<Entity extends AnyEntity, Where extends WhereOptions<Entity>>(
    entityRef: () => Entity,
    where: Where,
  ): Promise<number>
   ...
   */
  countBy<Entity extends AnyEntity, Options extends FindOptionsCount<Entity>>(
    entity: () => Entity,
    options: Options,
  ): Promise<number>
}
